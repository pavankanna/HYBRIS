...
public class UpdateOpenLigaDbService implements UpdateCompetitionService
{
	...
	public void update(final CompetitionModel competition)
	{
		...
		try
		{
			...
			for (final Matchdata match : response.getGetMatchdataByLeagueSaisonResult().getMatchdata())
			{
	...
	private MatchModel createMatch(final Matchdata data, final CompetitionModel competition)
	{
		...
		{
			match.setHomeGoals(Integer.valueOf(data.getPointsTeam1()));
			match.setGuestGoals(Integer.valueOf(data.getPointsTeam2()));
		}
		...
		LOG.info("Created new match " + match.getId() + " for " + competition.getCode());
		return match;
	}
	...
	private MatchModel updateMatch(final Matchdata data, final MatchModel match)
	{
		...
		{
			match.setDate(data.getMatchDateTime().toGregorianCalendar().getTime());
			message.append("  Updated date to ")
		...
		{
			match.setLocation(data.getLocation().getLocationCity() + "/" + data.getLocation().getLocationStadium());
			message.append("  Updated location to ").append(match.getLocation()).append("\n");
		}
		...
		if (data.isMatchIsFinished() && match.getHomeGoals() == null && result != null)
		{
			match.setHomeGoals(Integer.valueOf(result.getPointsTeam1()));
			message.append("  Updated home goals to ").append(match.getHomeGoals()).append("\n");
		}
		if (data.isMatchIsFinished() && match.getGuestGoals() == null && result != null)
		{
			match.setGuestGoals(Integer.valueOf(result.getPointsTeam2()));
			message.append("  Updated guest goals to ").append(match.getGuestGoals()).append("\n");
		}
