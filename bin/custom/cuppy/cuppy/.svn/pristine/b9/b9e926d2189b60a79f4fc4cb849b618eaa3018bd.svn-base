...
public class MatchesComponent extends Div implements BetPopupCallback
{
	...
	protected void renderMatchesRow(final Component parent, final MatchData matchData)
	{
		...
		// show date if needed
		final String day = DateFormat.getDateInstance(DateFormat.FULL, UISessionUtils.getCurrentSession().getLocale()).format(
				matchData.getDate());
		...
		timeLabelDiv.appendChild(new Label(DateFormat.getTimeInstance(DateFormat.SHORT,
				UISessionUtils.getCurrentSession().getLocale()).format(matchData.getDate())));
