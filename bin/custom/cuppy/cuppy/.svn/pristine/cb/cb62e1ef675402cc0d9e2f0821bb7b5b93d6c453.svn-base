...
public class MatchStatisticsComponent extends Div
{
	...
	private void updateLabel(final Div div)
	{
		...
		try
		{
			...
			label = new Label("Cat: "
					+ data.getMatch().getGroup().getName()
					+ "-"
					+ DateFormat.getDateTimeInstance(DateFormat.MEDIUM, DateFormat.SHORT,
							UISessionUtils.getCurrentSession().getLocale()).format(data.getMatch().getDate()));
			div.appendChild(label);
			...
			if (data.getMatch().isMatchFinished())
			{
				...
				final NumberFormat nbf = NumberFormat.getNumberInstance(UISessionUtils.getCurrentSession().getLocale());
