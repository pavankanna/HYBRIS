...
public class DefaultSecureTokenService implements SecureTokenService, InitializingBean
{
	...
	public String encryptData(final SecureToken data)
	{
		...
		try
		{
			...
			final byte[] md5SigBytes = generateSignature(unsignedDataBytes, 0, unsignedDataBytes.length, signatureKeyBytes);
	...
	private byte[] generateSignature(final byte[] data, final int offset, final int length, final byte[] signatureKeyBytes)
	...
	private boolean arrayEquals(final byte[] array1, final int offset1, final byte[] array2, final int offset2, final int length)
	{
	...
	private byte[] decrypt(final String encryptedText, final byte[] encryptionKeyBytes) throws GeneralSecurityException
	{
